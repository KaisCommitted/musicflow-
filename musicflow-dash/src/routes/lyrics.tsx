      ...
      <AnimatePresence mode="wait">
        {!jobId && folder && (
          <motion.button
            ...
            onClick={handleStart}
            disabled={starting}
            className="mt-6 flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground shadow-glow disabled:opacity-40"
          >
            <MicVocal className="h-4 w-4" />{" "}
            {starting ? "Starting…" : "Generate lyrics for library"}
          </motion.button>
        )}
